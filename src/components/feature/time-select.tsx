...
import { TimeSelectProps as NTimeSelectProps, TimeSelect as NTimeSelect } from '@nutui/nutui-react-taro';
...
export type TimeSelectProps = Omit<Partial<NTimeSelectProps>, 'value' | 'onChange'> & {
  // 触发器
...
} & IOptionsAPIProps<'value' | 'text' | 'children'>;

export const TimeSelect: FC<TimeSelectProps> = observer((props) => {
  const {
    value, valueType, placeholder = '请选择',
    options, api, keys, splitter, childrenKey = 'children',
    title, trigger, children,
    visible, onChange, onSelect, ...rest
  } = props;
  ...
  const change: TimeSelectProps['onChange'] = (val, selectedOptions) => {
    if (valueType === 'string' && Array.isArray(val)) {
